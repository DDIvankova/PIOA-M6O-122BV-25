...
class TestTable(unittest.TestCase):
    ...
    def test_create_record_with_custom_id(self):
        record = self.table.create(
            {"id": 100, "name": "Jane", "age": 30, "email": "jane@test.com"}
        )
    ...
    def test_create_extra_field(self):
        with self.assertRaises(ValidationError):
            self.table.create(
                {"name": "John", "age": 25, "email": "john@test.com", "extra": "value"}
            )
    ...
    def test_select_with_limit(self):
        for i in range(10):
            self.table.create(
                {"name": f"User{i}", "age": i, "email": f"user{i}@test.com"}
            )
    ...
    def test_update_id_to_duplicate(self):
        record1 = self.table.create(
            {"name": "John", "age": 25, "email": "john@test.com"}
        )
        record2 = self.table.create(
