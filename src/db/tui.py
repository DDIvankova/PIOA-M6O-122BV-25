...
class LibraryUI:
    def __init__(self):
        self.db = self._select_database_type()
        self._init_database()
    ...
    def _init_database(self):
        ...
        try:
            ...
            self.db.create_table("readers", readers_schema)
            self.db.create_table("loans", loans_schema)
            self._add_test_data()
        except (TableExistsError, FileExistsError):
            pass
    ...
    def _print_records(self, records: list[dict[str, Any]], title: str = "Записи"):
    ...
    def _read_sort_options(self) -> tuple[Optional[str], bool]:
        sort_by = self._read_optional_str(
            "Сортировать по полю (Enter - без сортировки): "
        )
        if not sort_by:
            return None, False
    ...
    def _add_book(self):
        ...
        result = self.db.insert_record("books", record)
    ...
    def _add_reader(self):
        ...
        result = self.db.insert_record("readers", record)
    ...
    def _view_books(self):
        ...
        if available:
            filters["is_available"] = available.lower() in (
                "доступна",
                "да",
                "true",
                "yes",
        ...
        try:
            records = self.db.select_records(
                "books", filters if filters else None, sort_by, reverse
            )
            self._print_records(records, "Книги")
        except (InvalidFieldError, InvalidSortError) as e:
            print(f"Ошибка: {e}")

    def _view_readers(self):
        ...
        try:
            records = self.db.select_records(
                "readers", filters if filters else None, sort_by, reverse
            )
            self._print_records(records, "Читатели")
        except (InvalidFieldError, InvalidSortError) as e:
            print(f"Ошибка: {e}")

    def _view_loans(self):
        print("\n--- Фильтрация выдач ---")
        print("Оставьте поле пустым, чтобы пропустить фильтр")
        filters = {}
        ...
        records = self.db.select_records("loans", filters if filters else None)
        enriched_records = []
        for loan in records:
            enriched = loan.copy()
            book = self.db.select_records("books", {"id": loan["book_id"]})
            reader = self.db.select_records("readers", {"id": loan["reader_id"]})
            if book:
                enriched["book_title"] = book[0]["title"]
            if reader:
                enriched["reader_name"] = (
                    f"{reader[0]['first_name']} {reader[0]['last_name']}"
                )
    ...
    def _update_book(self):
        ...
        book = self.db.select_records("books", {"id": record_id})
        if not book:
        ...
        if updates:
            result = self.db.update_record("books", record_id, updates)
    ...
    def _update_reader(self):
        ...
        reader = self.db.select_records("readers", {"id": record_id})
        if not reader:
        ...
        for key, value in reader.items():
            if key != "id":
                ...
                if new_value:
                    updates[key] = new_value
        if updates:
            result = self.db.update_record("readers", record_id, updates)
    ...
    def _delete_book(self):
        ...
        if confirm in ("да", "yes", "true", "1"):
            deleted = self.db.delete_record("books", record_id)
            print(f"Книга ID {deleted['id']} успешно удалена")
            related_loans = self.db.select_records("loans", {"book_id": record_id})
            for loan in related_loans:
                self.db.delete_record("loans", loan["id"])
            if related_loans:
    ...
    def _delete_reader(self):
        ...
        confirm = input("Подтвердите удаление (да/нет): ").strip().lower()
        if confirm in ("да", "yes", "true", "1"):
            deleted = self.db.delete_record("readers", record_id)
            print(f"Читатель ID {deleted['id']} успешно удален")
            related_loans = self.db.select_records("loans", {"reader_id": record_id})
            for loan in related_loans:
                self.db.delete_record("loans", loan["id"])
            if related_loans:
                print(f"Удалено связанных выдач: {len(related_loans)}")
    ...
    def _return_book(self):
        ...
        try:
            ...
            loan = self.db.select_records("loans", {"id": loan_id})
            if not loan:
            ...
            self.db.update_record("loans", loan_id, {"return_date": return_date})
            self.db.update_record("books", loan["book_id"], {"is_available": True})
    ...
    def _view_records_menu(self):
        ...
        try:
            if choice == "1":
                ...
                for name in self.db.get_table_names():
                    print(f"\n{name.upper()}:")
            else:
                print("Неверный выбор.")
